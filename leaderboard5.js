let filter = ''
let images = {
    "Early Supporter": "633344f6fc0ce323cf6aa045/638f7b8ba8b0110ca8d5667c_Aiddrop_Early-Supporter.png",
    ...
    "No Racism": "633404a85e0aa0228a89447f/635a46c2452f5ca1ab2c2272_NO-Racism_front.png",
        ...
        if (res.status === 200) {
            res.json().then(data => {
                ...
                donations.forEach((d) => {
                    ...
                    if (dict[d['Address']]) {
                        dict[d['Address']]['EthAmount'] = (parseFloat(d['EthAmount']) + (parseFloat(d['EuroAmount']) * parseFloat(d['ConversionRate']))).toFixed(3);
                        dict[d['Address']]['Euro'] = (parseFloat(dict[d['Address']]['Euro']) + parseFloat(d['EuroAmount'])).toFixed(2);
...
let start_document = async function () {
    await getting_data();
    ...
    if(variableValue != null && variableValue.toLowerCase() == 'skatedonate'){
        filter = 'Skate-Aid';
